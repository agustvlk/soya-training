...
class AddCurrency extends React.Component {
  ...
  successNotif = () => {
    if (this.state.isEdit) {
      NotificationManager.showSuccess("Currency saved");
    } else {
      NotificationManager.showSuccess("Currency added");
    }
  }
  ...
  showConfirmModal = async (id, name) => {
    ...
    ModalManager.show({
      ...
      footer: (
        <div>
          <Button variant='link-black' onClick={() => ModalManager.dismiss()}>Close</Button>
          ...
          this.state.currencies ?
            this.state.currencies.map(currency => (
              <tr key={currency.id}>
                  ...
                  <td><Link href={'currencies/edit?id=' + currency.id}>
                          <Button variant="green" iconProps={{ icon: 'edit' }}/>
                      </Link>
                      <Button variant="red" onClick={() => this.showConfirmModal(currency.id, currency.name)}
